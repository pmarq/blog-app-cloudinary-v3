...
export default function StudioCmoDraftPage() {
  const searchParams = useSearchParams();
  const router = useRouter();
  const { toast } = useToast();

  const briefId = firstValue(searchParams.get("briefId") ?? "");
  const orgId = firstValue(searchParams.get("orgId") ?? "");
  const briefTitle = firstValue(searchParams.get("briefTitle") ?? "");
  const briefObjective = firstValue(searchParams.get("briefObjective") ?? "");
  const briefAngle = firstValue(searchParams.get("briefAngle") ?? "");
  const briefCta = firstValue(searchParams.get("briefCta") ?? "");
  const briefChannel = firstValue(searchParams.get("briefChannel") ?? "");
  const briefTheme = firstValue(searchParams.get("briefTheme") ?? "");
  const briefScope = firstValue(searchParams.get("briefScope") ?? "");
  ...
  const loadExistingContent = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      setContent(latest);
      setWarning(latest.warning || null);
      setMeta({
        ...
        warnings: latest.warning ? [String(latest.warning)] : [],
      });
      return true;
    } catch {
      return false;
  ...
  const generateContent = useCallback(async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "O texto foi criado e pode ser revisado antes do editor final.",
      });
    } catch (err) {
      ...
      <div className="max-w-5xl space-y-6">
        ...
        <div className="space-y-1">
          ...
          <p className="text-sm text-secondary-dark dark:text-secondary-light">
            Etapa intermediária antes do editor final. Aqui a IA gera e você revisa.
          </p>
        </div>
        ...
        <section className="rounded border border-secondary-dark/20 dark:border-secondary-light/20 bg-secondary-light/10 dark:bg-secondary-dark/20 p-4 space-y-4">
          <div className="flex flex-wrap items-start justify-between gap-3">
            ...
            <div className="rounded border border-secondary-dark/20 dark:border-secondary-light/20 bg-white/50 dark:bg-black/10 px-3 py-2 text-xs text-secondary-dark dark:text-secondary-light">
              ...
              <div>{briefTheme || briefScope || "Rascunho editorial"}</div>
            </div>
          </div>
        ...
        {status || loadingExisting ? (
          <div className="flex items-center gap-2 text-xs text-secondary-dark dark:text-secondary-light">
            ...
            {status || "Carregando rascunho salvo..."}
          </div>
        ) : null}
